...
library(tidyverse)
experts_and_insights <- read_csv("/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/experts_insights_combined.csv")
...
```{r}
ggAcf(Dami$start/1000, type = "correlation") + ggtitle("Autocorrelation Plot")
...
```{r}
ggAcf(Dami_diff$first_diff, type = "correlation") + ggtitle("Autocorrelation Plot")
...
adf.test(Dami_diff_na$first_diff)
...
ggAcf(residuals(cookie_model), type="correlation")
...
```{r}
autoplot(forecast(cookie_model,h=5, level = 80))
```
